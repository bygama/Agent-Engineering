...
`<repo-path>` is the repo being audited). It settles the mechanical checks
(budgets, pointer shape, stamp, adapters, read orders, links, lanes,
feature-list schema/regression, DESIGN drift, command drift) so the audit
spends judgment on the rest. Fold its findings into the report.
