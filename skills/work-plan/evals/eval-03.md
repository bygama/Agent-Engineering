...
shape fan-out expects at intake.
...
- [ ] Answers the three fan-out questions in writing inside the parent
      PLAN: where does each item's work happen, how do the results
      merge, who resolves disagreement.
- [ ] The parent PLAN.md gains a worker table skeleton (item · lane
      `work/<slug>/` · worktree path · branch · runner · spawn
      command) shaped for fan-out to consume, not left as an empty
      heading.
...
- [ ] Names fan-out as the mechanism that executes this parent plan;
